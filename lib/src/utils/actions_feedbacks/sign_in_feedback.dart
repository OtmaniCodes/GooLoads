...
class SignInFeedback {
  static showSignInFeedback(BuildContext context) {
    showDialog(
      context: context,
      builder: (_) {
          ...
          builder: (context, authState) {
            if (authState is AuthenticationLoading) {
              ...
              if (authState.isSuccess) {
               ...
               String userName = serviceLocator<AuthService>().getUserCredentials()?.displayName??"User";
                return _getFeedbackContent(context, "Welcome back $userName, You successfully signed in!", 1);
              } else {
  ...
  static Widget _getFeedbackContent(BuildContext context, String message, int index) {
    const List<IconData> myIcons = [
      Icons.loop_rounded,
      Icons.login_rounded,
      Icons.error
    ];
    if (index == 1) Future.delayed(Duration(seconds: 2), () => Navigator.pop(context));
